       ...
       {  static int i=0;
          ...
          switch(SelectedWord[i+1])
          {
           ...
           {  
               ...
               case 'A' : DrawLineEx(Avec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
               if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl2(SelectedWord,Avec,Bvec,Cvec,Mvec);}
               break;
               case 'S' : DrawLineEx(Bvec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
               if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl2(SelectedWord,Avec,Bvec,Cvec,Mvec);}
               break;
               case 'K' : DrawLineEx(Cvec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
               if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl2(SelectedWord,Avec,Bvec,Cvec,Mvec);}
               break;
