       ...
       {   
          ...
          switch(SelectedWord[i+1])
          {
           ...
           {  
               ...
               case 'A' : if((WhichİsUsed==1&&Another==0)||(WhichİsUsed==2&&Another!=0)) {DrawLineEx(Avec,temp,20,(Color){0,0,0,150}); DrawCircleV(temp,20,(Color){0,0,0,150});
           if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl4(SelectedWord,Avec,Bvec,Cvec,Dvec,Mvec);}}
              else {
                DrawLineEx(Bvec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
              if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl4(SelectedWord,Avec,Bvec,Cvec,Dvec,Mvec);}}
               break;
               case 'D' : DrawLineEx(Cvec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
               if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl4(SelectedWord,Avec,Bvec,Cvec,Dvec,Mvec);}
               break;
               case 'M' : DrawLineEx(Dvec,temp,20,(Color){0,0,0,150});DrawCircleV(temp,20,(Color){0,0,0,150});
               if(temp.x!=Mvec.x&&temp.y!=Mvec.y){i++;CizgiCizLvl4(SelectedWord,Avec,Bvec,Cvec,Dvec,Mvec);}
               break;
          }
          }
